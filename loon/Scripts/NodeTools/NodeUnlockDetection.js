...
// ChatGPT
const GPT_BASE_URL = 'https://chatgpt.com/cdn-cgi/trace'
// Claude
const CLAUDE_BASE_URL = 'https://claude.ai/cdn-cgi/trace'
...
Promise.all([ytbTest(),disneyLocation(),nfTest(),daznTest(),parmTest(),discoveryTest(),gptTest(),claudeTest()]).then(value => {
    let content = "------------------------------------</br>"+([result["Dazn"],result["Discovery"],result["Paramount"],result["Disney"],result["Netflix"],result["ChatGPT"],result["Claude"],result["YouTube"]]).join("</br></br>")
...
function claudeTest() {
    return new Promise((resolve, reject) => {
        ...
        $httpClient.get(params, (errormsg, response, data) => {
            console.log("----------Claude--------------");
            if (errormsg) {
                console.log("Claude request failed:" + errormsg);
                result["Claude"] = "<b>Claude: </b>检测失败 ❗️";
                resolve(errormsg);
                return;
            }
            if (data && data.indexOf("loc=") !== -1) {
                let region = data.split("loc=")[1].split("\n")[0].trim()
                console.log("Claude Region: " + region)
                let res = support_countryCodes.indexOf(region)
                if (res != -1) {
                    result["Claude"] = "<b>Claude: </b>支持 " + arrow + "⟦" + flags.get(region.toUpperCase()) + "⟧ 🎉"
                    console.log("支持 Claude")
                } else {
                    result["Claude"] = "<b>Claude: </b>未支持 🚫"
                    console.log("不支持 Claude")
                }
                resolve(region)
            } else {
                result["Claude"] = "<b>Claude: </b>检测失败 ❗️"
